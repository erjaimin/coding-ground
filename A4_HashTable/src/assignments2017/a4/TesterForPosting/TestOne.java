...
import hashMap.Song;
import hashMap.MyHashTable;

import java.util.ArrayList;

public class TestOne extends Test {
	private ArrayList<Song> songs;
	private static String dataFile = "/Users/i858280/Documents/workspace/jpws/A4_HashTable/src/assignments2017/a4/TesterForPosting/songs.txt"; // NEED
																																				// TO
																																				// ADD
																																				// DIRECTORY
	...
	public Integer tester() {
		...
		try {
			for (int test = 0; test < testCount; test++) {
				...
				if (!mySong.getArtist().equals(javaSong.getArtist()) || !(mySong.getYear() == javaSong.getYear())) {
			...
			mySongTable.rehash();
			if ((mySongTable.getNumBuckets() != 2 * oldBucketCount) || (mySongTable.size() != oldSize)) {
				scoreb -= 5;
			...
			if (testSongChange == null || !testSongChange.getArtist().equals("Yellow Flag")
					|| testSongChange.getYear() != 1984) {
				this.out.println("Failed to update old value for key in put.");
				scoreb -= 5;
			}
			if (oldScream == null || !oldScream.getArtist().equals(expectedOldSong.getArtist())
		...
		try {
			...
			for (int test = 0; test < testCount; test++) {
				...
				if (mySong == null && javaSong == null) {
					continue;
				} else if ((mySong == null && javaSong != null) || (mySong != null && javaSong == null)) {
					this.out.println("Expected: " + javaSong);
					this.out.println("Output: " + mySong);
					scorec -= 12;
					break;
