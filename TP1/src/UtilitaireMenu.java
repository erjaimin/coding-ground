...
public class UtilitaireMenu {
	 ...
	 */
	public static boolean gererMenu(Carte[] jeuNeuf, Carte[] cartes,
		 ...
		 * possible de r�utiliser en une seule ligne de code (voir:
		 * montrer_stats).
		 * 
		 * Doit �tre modifi� si on ajoute des options de menu dans le
		 * tableau-constante TAB_OPTIONS_MENU.
		 */
		...
		if (reponse.equals(
				Constantes.TAB_OPTIONS_MENU[Constantes.NOUVELLE_PARTIE])) {
			UtilitaireStats.ajusterStatsNouvellePartie(stats);
			UtilitaireJeu.initialiserJeu(jeuNeuf, cartes, gui, etatJeu);
		}

		else if (reponse
				.equals(Constantes.TAB_OPTIONS_MENU[Constantes.CARTE])) {

			UtilitaireJeu.montrerLesCartes(cartes, gui, etatJeu);
		}

		else if (reponse
				.equals(Constantes.TAB_OPTIONS_MENU[Constantes.INDICE])) {
			UtilitaireJeu.montrerIndices(cartes, gui, etatJeu);
		}

		else if (reponse
				.equals(Constantes.TAB_OPTIONS_MENU[Constantes.STATS])) {

			UtilitaireStats.montrerStats(stats);
		}

		else if (reponse
				.equals(Constantes.TAB_OPTIONS_MENU[Constantes.QUITTER])) {
